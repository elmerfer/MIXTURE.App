...
# dev.off()
saveRDS(df.BlandAltman.n, file = "DF_BlandLtman.Noise.LM22.Figura2a.RDS")

saveRDS(TIL10.df.BlandAltman.n, file = "DF_BlandLtman.Noise.TIL10.Figura2a.RDS")
...
beta.length.noise <- apply(Beta.n ,1, function(x) sum(x>0))
...
TIL10.cib.bet.length.noise <- rowSums(TIL10.noise.cib>0)
...
# saveRDS(df.betas.noise, file = "DF_BoxplotsNumberCoefs_Noise.LM22.Figura1.d.RDS")
# saveRDS(TIL10.df.betas.noise, file = "DF_BoxplotsNumberCoefs_Noise.TIL10.Figura1.d.RDS")
...
bx2 <- ggplot(TIL10.df.betas.noise, aes(x=beta, y=est)) +
  ...
  labs( x = "True number of coefficients",
        ...
        fill = "Algorithm") + theme_bw() +   
  scale_y_continuous(name ="Estimated number of coefficients", 
  breaks = c(0:10),labels=as.character(c(0:10)))#+ facet_wrap(~Signature) 

bx2
# +
#   scale_y_discrete(name ="Estimated number of coefficients", 
#                    limits=as.character(c(0:10)))#+ facet_wrap(~Signature) 
setEPS()
...
## without noise FIGURA 2 Supplementary Figure 2

##Number of estimated cell types
beta.length <- apply(Beta,1, function(x) sum(x>0))
TIL10.beta.length <- apply(TIL10.Beta ,1, function(x) sum(x>0))

##number of estimated betas
cib.bet.length <- rowSums(M.r.cib>0)
TIL10.cib.bet.length <- rowSums(TIL10.pure.cib>0)

robust.bet.length <- rowSums(GetMixture(MIX,"prop")>0)
TIL10.robust.bet.length <- rowSums(GetMixture(TIL10.MIX,"prop")>0)

abbas.bet.length <- rowSums(GetMixture(ABBAS.pure,"prop")>0)
TIL10.abbas.bet.length <- rowSums(GetMixture(TIL10.ABBAS.pure,"prop")>0)


# dt.length.noise <- rowSums(dt_out.noise$estimates > 0) 

abis.bet.length <- rowSums(GetMixture(ABIS.pure,"prop")>0)
TIL10.abis.bet.length <- rowSums(GetMixture(TIL10.ABIS.pure,"prop")>0)

quanti.bet.length <- rowSums(pure.quanti[,-c(1,24)] >0)
TIL10.quanti.bet.length <- rowSums(TIL10.pure.quanti[,-c(1,12)] >0)

df.betas <- data.frame(est = c(cib.bet.length,
                                     robust.bet.length,
                                     abbas.bet.length,
                                     # dt.length.noise,
                                     abis.bet.length,
                             ...
                             method = factor(c(rep("CIBERSORT", length(cib.bet.length)), 
                                               rep("MIXTURE",length(robust.bet.length)),
                                               rep("ABBAS",length(abbas.bet.length)),
                                               # rep("DTANGLE",length(beta.length.noise)),
                                               rep("ABIS",length(abis.bet.length)),
                                               rep("QUANTISEQ",length(quanti.bet.length))
                             ),levels = c("ABBAS","ABIS","QUANTISEQ","CIBERSORT","MIXTURE")))
df.betas$Signature <- "LM22"

TIL10.df.betas <- data.frame(est = c(TIL10.cib.bet.length,
                                           TIL10.robust.bet.length,
                                           TIL10.abbas.bet.length,
                                           # dt.length.noise,
                                           TIL10.abis.bet.length,
                                           TIL10.quanti.bet.length),
                                   beta = as.factor(rep(TIL10.beta.length,5)),
                                   method = factor(c(rep("CIBERSORT", length(TIL10.cib.bet.length)), 
                                                     rep("MIXTURE",length(TIL10.robust.bet.length)),
                                                     rep("ABBAS",length(TIL10.abbas.bet.length)),
                                                     # rep("DTANGLE",length(beta.length.noise)),
                                                     rep("ABIS",length(TIL10.abis.bet.length)),
                                                     rep("QUANTISEQ",length(TIL10.quanti.bet.length))
                                   ),levels = c("ABBAS","ABIS","QUANTISEQ","CIBERSORT","MIXTURE")))
TIL10.df.betas$Signature <- "TIL10"
...
scale_y_continuous(name, breaks, labels, limits, trans)
bx2 <- ggplot(TIL10.df.betas, aes(x=beta, y=est)) +
  geom_boxplot(position = position_dodge(width = 0.7), aes(colour = method) ) +
  labs( x = "True number of coefficients",
        y = "Estimated number of coefficients", 
        fill = "Algorithm") + theme_bw() +
...
pdf("/home/elmer/Dropbox/IDEAS/cibersort/GenomeR/Figuras/FigurasFinales/SupFIG2_betas.PURE.pdf", paper = "a4",
    width = 10, height = 10)
png("/home/elmer/Dropbox/IDEAS/cibersort/GenomeR/Figuras/FigurasFinales/SupFIG2_betas.PURE.png")
grid.arrange(bx1, bx2, nrow = 2)
dev.off()
